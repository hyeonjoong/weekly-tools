...
``statwise-그룹비교통계`` tool (this copy uses ``norm_sf`` for the p-value).

Royston fitted the approximation for 3 <= n <= 5000; ``MAX_RELIABLE_N`` records
that so callers can warn rather than silently extrapolating.
...
from .special import norm_ppf, norm_sf

__all__ = ["shapiro_wilk", "MAX_RELIABLE_N"]
...
def shapiro_wilk(x: Sequence[float]) -> Tuple[float, float]:
    ...
    z = (y - m_mu) / s_sigma
    # norm_sf, not 1 - norm_cdf: the subtraction loses the whole tail above
    # z ~ 8 and returned a literal p = 0.0 for strongly non-normal samples.
    p = norm_sf(z)
    return w, max(0.0, min(1.0, p))
