...
- **상관·추출·회전**: 피어슨 또는 **폴리코릭(순서형 리커트 잠재상관)** 상관행렬(`--correlation`) →
  **주성분(PCA)** 또는 **주축분해(PAF, 공통요인 모형)** 추출(`--extraction`) + Varimax 직교회전 /
...
- **역문항 재점수화**, 결측(listwise) 처리, 인코딩 지정(`--encoding`, CP949 등), JSON 출력,
  **논문 부록용 적재표 CSV 내보내기**(`--csv-out`, utf-8-sig)

> 추출은 기본이 **주성분(PCA)** 으로 SPSS 요인분석의 기본 추출과 동일합니다. 공통요인 모형이 필요하면
> `--extraction paf`(주축분해, SMC 초기·반복)를 쓰세요 — ω·RMSR이 덜 낙관적으로 나옵니다(아래 한계 참고).
...
`--extraction pca|paf`(추출 방식), `--rotation varimax|promax|none`, `--reverse Q1,Q2 --scale-min 1 --scale-max 5`(역문항),
`--parallel-iter N`(평행분석 반복, 0이면 생략), `--seed`(재현), `--min-loading 0.40`(주적재/교차적재 임계값, 0~1),
`--na 값`(결측 문자열 추가), `--encoding cp949`(한국어 엑셀 CSV 등 UTF-8이 아닌 파일),
`--csv-out 경로`(문항×요인 적재표+요약행을 CSV로 — 논문 부록·엑셀용),
`--scores-out 경로 --score-method sum|mean`(응답자별 하위척도 점수를 CSV로 — 후속 타당도 분석용).

> **CSV 내보내기**: `--csv-out` 은 문항별 적재·공통성·MSA·문항-총점 + **요약행**(적재제곱합·설명분산%·누적%·
> ω·α, 사교회전이면 요인상관 Φ)을 담아 표 하나로 완결됩니다(집계값만, 개인정보 없음). `--scores-out` 은
> 요인분석에 실제 쓰인(결측제거) 응답자마다 하위척도 합/평균 점수를 ID와 함께 내보내, 요인구조 확인 후
> 곧바로 집단비교·수렴타당도 분석에 넘길 수 있습니다(역문항 재점수화 반영). 두 파일 모두 utf-8-sig(BOM)라
> 한국어 엑셀에서 바로 열립니다.
>
...
- 추출은 **주성분(PCA, 기본)** 과 **주축분해(PAF, `--extraction paf`)** 를 제공합니다. PCA는 SPSS 기본과
  동일하고, PAF는 SMC 초기·반복 방식의 공통요인 모형입니다(ML은 미지원 — 필요하면 `factor_analyzer` 병행).
  PAF에서 공통성이 1을 넘는 **Heywood 케이스**는 1로 절단하고 경고합니다(요인 수 과다·표본 부족 신호).
