...
- **귀무모형 선택 — 색인 밀도 보정(`--gap-null degree`)**: 기본 초기하 검정은 "주제 B 가
  어느 논문에 붙을 확률은 모든 논문이 같다"고 가정합니다. 실제 색인은 그렇지 않아
  (주제어 3개짜리 논문과 25개짜리 논문이 섞여 있습니다) 그 가정이 깨집니다. `degree` 를
  주면 코퍼스의 표목 수를 그대로 둔 채 B 를 표목 수에 비례해 재배분하는 귀무모형
  (`p_i = min(1, c_B·(m_i−1)/(M−c_A))`, A 를 단 논문 i 에 대해)으로 기대값을 다시 계산하고,
  **포아송이항 정확검정**으로 p 를 냅니다. 주제어가 하나뿐인 논문은 어떤 쌍도 가질 수
  없으므로 기대에서 빠집니다(아주 흔한 주제에서는 `p_i` 가 1 로 잘려 주변합이 정확히
  보존되지는 않는데, 그만큼 공백을 **덜** 주장하는 보수적 방향입니다).
  ⚠️ **이것은 '헛공백 제거 필터'가 아니라 두 번째 의견입니다** — 앵커 주제(둘 중 논문이
  적은 쪽)의 논문들이 코퍼스 평균보다 얇게 색인돼 있으면 공백이 약해지고, **두껍게
  색인돼 있으면 오히려 강해집니다**(실측: q=0.22 이던 쌍이 q=8e-08 로 바뀌는 코퍼스를
  만들 수 있습니다). 두 모형에서 **모두 살아남는 공백**만 근거로 쓰세요. 독립가정
  기대값은 JSON/CSV 의 `expected_independent` 에 나란히 실려 비교할 수 있습니다.
  리포트 머리글의 **논문 1편당 평균 주제어 수**가 이 옵션을 고를 때의 관측치입니다.
  기대값은 둘 중 **논문이 적은 쪽**을 조건으로 계산합니다 — 어느 쪽을 잡느냐로 기대값이
  8배 넘게 갈릴 수 있어(실측 2.87 vs 26.77) 자의적 선택이 결론을 좌우하지 않도록
  규칙으로 고정했습니다. 아주 흔한 주제(코퍼스 절반 이상)와 평균보다 훨씬 깊게 색인된
  논문이 만나 **모형이 포화**되는 쌍(논문 하나에 그 주제를 한 번 넘게 넣어야 하는 상황)은
  자동으로 **독립가정으로 되돌려** 검정하고 표에 `⚠밀도모형 포화→독립가정` 으로 표시합니다
  — 잘라낸 채로 밀도 모형을 쓰면 분산이 사라져 `p=0` 짜리 가짜 1순위 공백이 나오기 때문입니다.
- **취약도(fragility) — "이 공백은 논문 몇 편에 매달려 있나"**: 유의한 공백마다
  **함께 다룬 논문이 몇 편만 더 있었어도 q>0.05 가 되어 사라졌을지**를 셉니다
  (임상시험의 **fragility index** 와 같은 발상, 단 여기서는 다중검정 보정 후의 q 기준).
  `+1편` 이면 색인 한 건에 결론이 매달려 있다는 뜻이고, `≥N편` 은 **N편을 얹어 보고도**
  뒤집히지 않았다는 뜻입니다(N 은 실제로 시험한 편수 — 최대 40편, 관측 상한이 더 낮으면 그만큼).
  요약 블록의 1순위 후보에도 함께 표시되어, 표까지 내려가지 않아도 견고성을 알 수 있습니다.
  ⚠️ q 기준이므로 **검정군 크기(`--gap-top-k`)가 바뀌면 값도 바뀝니다** — 같은 설정 안에서
  후보끼리 비교하는 상대 지표로 쓰고, 논문에 절대 수치로 인용하지 마세요.
...
- 출력: **Markdown**(기본, ~220줄), **JSON**(`--format json`), **CSV**(`--format csv` —
...
# 6-1) 색인 밀도를 보정한 귀무모형으로 다시 계산해 비교 (제거가 아니라 두 번째 의견)
pubgap --from-file examples/sleep_export.csv --gap-null degree
#      취약도가 실제 숫자로 찍히는 예(검정군을 줄이면 q 가 좋아집니다):
pubgap --from-file examples/sleep_pubmed.xml --gap-top-k 4
...
- 공백 기준/정렬: `--gap-min-expected`(기본 2.0), `--gap-max-lift`(기본 0.5),
  ...
  `--gap-null {independent,degree}`(귀무모형 — 기본 `independent`(초기하),
  `degree` 는 색인 밀도 보정 + 포아송이항 정확검정).
...
> 각 표의 설명 각주, `### 실행 정보` 가 더 있습니다(전체 약 220줄).
...
| 주제 A | 주제 B | 함께(관측) | 기대 | 부족 | lift | 95% CI | p | q(FDR) | 취약도 | 추이 |
|---|---|---:|---:|---:|---:|:--:|---:|---:|:--:|:--:|
...
_`취약도` = **함께 다룬 논문이 몇 편만 더 있었어도 q>0.05 가 되어 이 공백이 사라졌는가**(임상시험의 fragility index 와 같은 발상). `+1편` 이면 색인 한 건에 결론이 매달려 있다는 뜻이고, 숫자가 클수록 견고합니다. `–` 는 애초에 q>0.05 라 물을 수 없는 줄입니다._
_귀무모형: 독립가정(초기하). 논문마다 주제어 수가 크게 다른 코퍼스에서는 `--gap-null degree` 로 **색인 밀도를 보정한** 기대값과 비교해 보세요 — 얇게 색인된 코퍼스에서는 독립가정이 공백을 과대평가합니다._
...
| 기대 동시등장 | `count(A)·count(B) / N` (독립 가정 = `--gap-null independent`, 기본) |
| 기대 동시등장 (`--gap-null degree`) | 앵커 A(둘 중 논문이 **적은** 쪽, 동수면 이름 오름차순)를 단 논문 i 마다 `p_i = min(1, count(B)·(m_i−1)/(M−count(A)))` 를 매기고 그 합. `m_i`=논문 i 의 주제어 수, `M`=전체 표목 수. 주제어가 하나뿐인 논문은 기여 0. **자르기 전** `p_i > 1` 인 쌍은 모형이 성립하지 않으므로(포화) 그 쌍만 독립가정으로 되돌리고 표에 표시 |
| lift | `관측 동시등장 / 기대` — 낮을수록 미개척. `기대`·`부족`·`lift`·`p` 는 모두 **선택한 귀무모형**의 값이고, 독립가정 기대값은 JSON/CSV 의 `expected_independent` 에 별도로 실린다 |
...
| p-value | 초기하분포 하단꼬리 `P(X ≤ 관측)` — 우연히 이만큼 덜 엮일 확률(작을수록 유의). `--gap-null degree` 에서는 위 `p_i` 들의 **포아송이항 하단꼬리**(절단 DP 로 정확 계산) |
| 취약도(fragility) | `q ≤ 0.05` 인 공백에 대해, 다른 쌍의 p 를 고정한 채 그 쌍의 관측만 1편씩 올려 **q > 0.05 가 되는 최소 추가 편수**. 최대 40편(또는 관측 상한 `min(count(A),count(B))`)까지 시험하고, 그 안에 안 뒤집히면 `≥N편`. q 기준이라 검정군 크기(`--gap-top-k`)에 따라 값이 달라진다 |
