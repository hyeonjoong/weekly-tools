...
DAU/WAU/MAU · 리텐션(신뢰구간 포함) · 퍼널 전환(전환율 신뢰구간 + 단계별 소요시간) ·
프로토콜 준수도(adherence) · 활동 시간대**를
한 번에 계산해 텍스트 또는 **JSON**으로 보여줍니다. 로그에 **군(arm) 열**이 있으면
`--group-col` 하나로 **중재군 vs 대조군 비교**(리텐션·준수율 차이 + Newcombe 신뢰구간 ·
Fisher 정확검정 · Mann-Whitney · 이탈 Kaplan-Meier/log-rank · Holm 다중비교 보정)까지 함께 냅니다.
리포트를 외부와 공유해야 하면 `--anonymize` 로 사용자 ID를 가명(U001…)으로 바꿔 출력합니다.
...
**프로토콜 준수도 / Adherence** — DTx·앱 기반 중재 연구의 프로토콜에는 보통
"8주간 **주 5일 이상** 사용" 처럼 *기간 단위의 사용 빈도* 가 적혀 있고, 결과 표에는
주차별 준수율·준수 참여자 비율이 들어갑니다. `--adherence-days 5` 하나면 이 표가 나옵니다.
연구 주차는 달력 주가 아니라 **참여자별 첫 활동일** 기준으로 끊고, 관찰 종료일에 걸친
**부분 주는 분모에서 뺍니다** — "아직 쓸 시간이 없었던" 것을 미준수로 세지 않기 위함입니다.

**언제 쓰나 / When** — 사용자 로그가 손에 있고, 빠르게 활성도·잔존율·퍼널·준수율을 보고 싶을 때.
...
# 프로토콜 준수도 — "주 5일 이상 사용" 규약을 8주까지
logflow examples/trial_events.csv --adherence-days 5 --adherence-weeks 8

# 준수율까지 군 비교에 넣기 (준수 참여자 비율 + 사용자당 준수 주 비율이 검정에 추가됨)
logflow examples/trial_events.csv --group-col arm --ref-group control --adherence-days 3

# 리포트를 외부와 공유해야 할 때: 사용자 ID를 U001··· 가명으로
logflow my_log.csv --anonymize --out 공유용_리포트.txt
```
...
[ 프로토콜 준수도 ]  ← examples/trial_events.csv --adherence-days 3 의 출력 (각주 일부 생략)
  관찰 종료일     : 2026-03-22 (로그 전체의 마지막 활동일 — 모든 '완전히 관찰된 주' 판정의 기준)
  관찰 창         : 3주 (참여자마다 완전히 관찰된 주만 · 최대 3주)
  준수 참여자     : 5/24  (20.8%, 95%CI 9.2–40.5%)  ← 자기 관찰 주의 80% 이상 준수
    └ 분석 집단   : 관찰 주가 1개 이상인 참여자 (분모가 된 관찰 주 수: 2~3주)
  사용자당 준수율 : 중앙값 50.0% (참여자별 준수 주÷관찰 주)
  연속 준수 주    : 중앙값 1.0주 (참여자별 '가장 긴 연속 준수' 의 중앙값)
  주차        대상    준수    준수율  95%CI                 활성일(중앙)
  1             24      18     75.0%  [55.1, 88.0]%                  4.0
  2             24       6     25.0%  [12.0, 44.9]%                  0.0
  3              7       1     14.3%  [2.6, 51.3]%                   0.0
  ! 참여자마다 관찰된 주 수가 다릅니다(2~3주). ... --adherence-weeks 로 창을 고정해
    완주자만 비교하세요.

    ↓ --adherence-weeks 3 을 더하면 3주를 끝까지 관찰한 완주자만 분모가 됩니다
  준수 참여자     : 1/7  (14.3%, 95%CI 2.6–51.3%)
    └ 분석 집단   : 3주 완주자 (분모가 된 관찰 주 수: 3주)
...
[ 군 비교 ] (열: arm, 기준군: control)
  ...
  * 다중비교: 이 절의 검정 8개를 하나의 family 로 보고
    ...
    (검정 개수는 --retention/--funnel/--adherence-days 에 따라 달라지므로, 보정된 p 값도
...
| `--adherence-days` | 프로토콜 준수 기준: 한 주에 이 일수 이상 사용하면 준수(`1 ≤ N ≤ --adherence-period`). 주면 준수도 분석 켜짐 | 없음(off) |
| `--adherence-period` | 준수도의 '한 주' 길이(일). 격주 규약이면 `14`. *(`--adherence-days` 와 함께만)* | `7` |
| `--adherence-target` | '준수 참여자' 판정: 자기 관찰 주의 이 비율 이상 준수 (0 초과 1 이하). *(위와 함께만)* | `0.8` |
| `--adherence-weeks` | 프로토콜 관찰 창(주차 수, 1~104). 주면 **창을 끝까지 관찰한 완주자만** 분모. *(위와 함께만)* | 관찰된 만큼(≤104주) |
| `--anonymize` | 사용자 ID를 `U001…` 가명으로 바꿔 출력(대응표는 저장 안 함) | off |
...
- **CSV 표 내보내기(`--csv-dir`)**: DAU/WAU/MAU·리텐션(신뢰구간 포함)·이벤트·사용자·활동
  ...
  `adherence_weekly`(`week` 열은 `period_days` 일 단위)·`adherence_users` 는
  `--adherence-days` 를 준 경우에만,
...
- **프로토콜 준수도(`--adherence-days N`)**: "한 주(기본 7일)에 N일 이상 사용" 규약의 이행 정도.
  - **연구 주차는 참여자별 첫 활동일(day 0) 기준**으로 끊습니다(달력 주가 아님). 등록 시점이
    다른데 달력 주로 끊으면 누군가의 1주차가 3일짜리 부분 주가 되어 준수율이 낮게 잡힙니다.
  - **완전히 관찰된 주만 분모(대상)** 입니다. 관찰 종료일에 걸친 마지막 부분 주는 제외 —
    "아직 다 쓸 시간이 없었다"를 "안 썼다"로 기록하지 않기 위함입니다. 그래서 뒤쪽 주차일수록
    대상 인원이 줄어듭니다(늦게 등록한 사람은 그 주차를 관찰할 기회가 없었음). 즉 **주차별
    표는 뒤로 갈수록 앞 주차의 부분집합**이라, 주차 간 추세에는 코호트 구성 변화가 섞입니다.
  - **활성일** = 이벤트가 하나라도 있던 날의 수. 하루에 몇 번 썼든 1일로 셉니다("주 5일" 문언
    그대로). 표의 `활성일(중앙)` 은 **대상 전원(그 주 사용 0일인 사람 포함)** 의 중앙값이라,
    준수율이 25%인 주에 0.0 이 나올 수 있습니다.
  - **사용자당 준수율** = 참여자별 (준수 주 ÷ 자기 관찰 주)의 중앙값.
    **연속 준수 주** = 참여자별 *가장 긴* 연속 준수 구간의 중앙값(평균 구간 길이가 아닙니다).
  - **준수 참여자 — 분모가 중요합니다.**
    - `--adherence-weeks N` 을 **주면**: 그 N주를 **끝까지 관찰한 완주자만** 분모입니다.
      중도에 관찰이 끝난 사람은 빠지고(인원을 `!` 로 알림), 완주자가 없으면 계산하지 않습니다.
      **군 비교나 논문 보고에는 이쪽을 쓰세요.**
    - **주지 않으면**: 참여자마다 자기가 관찰된 주 수를 분모로 씁니다. 관찰이 짧을수록 기준을
      넘기 쉬워(1주 중 1주 = 100%) 이 비율이 행동이 아니라 **등록 시점을 반영**할 수 있습니다.
      분모가 서로 다르면 `!` 로 경고하고 그 범위(예: `2~3주`)를 함께 표시합니다.
    - 판정 기준은 `--adherence-target`(기본 0.8, **0 초과 1 이하**). 관찰 주가 적으면 이 값은
      사실상 올림됩니다 — 관찰 2주에 target 0.8 이면 2/2, 즉 **100%** 를 요구합니다.
      주차 수가 적은 데이터에서는 `준수 참여자` 보다 `주차별 준수율`·`사용자당 준수율` 을
      보고하는 편이 정확합니다.
  - `--adherence-period` 로 기간 단위를 바꿉니다(격주 규약이면 `14`). 이때 리포트의 '주' 표기는
    '기간' 으로 바뀌지만 `adherence_weekly.csv` 의 열 이름은 `week` 로 유지되며, 같은 행의
    `period_days` 열이 실제 길이를 알려 줍니다.
  - `--adherence-days` 는 `1 ≤ N ≤ --adherence-period` 여야 하고, `--adherence-period` ·
    `--adherence-target` · `--adherence-weeks` 는 **`--adherence-days` 와 함께만** 씁니다.
    주차 표는 어느 경우든 **104주까지만** 만들며(그 이상은 알림), `--adherence-weeks` 도
    1~104 범위입니다.
  - **관찰 종료일이 곧 분모입니다.** 리포트 첫 줄에 `관찰 종료일` 을 표시하니 꼭 확인하세요 —
    로그에 오타로 미래 날짜가 한 줄 섞이면 그 뒤의 빈 주가 모두 '미준수' 로 잡혀 준수율이
    급락합니다. `--only-groups` 나 `--from/--to` 로 데이터를 줄이면 이 종료일도 함께
    바뀌므로(다른 지표와 동일) 준수율이 달라질 수 있습니다.
  - **한계**: (1) 여기서의 '준수'는 *로그에 활동이 남았는가* 이지 중재를 제대로 수행했는지가
    아닙니다 — 완수 이벤트 기준으로 보려면 로그를 그 이벤트만 남기고 거르거나 퍼널 완주율을
    함께 보세요. (2) 주차 1의 시작이 **첫 로그 활동일**이라, 등록 후 한동안 아예 쓰지 않은
    기간은 보이지 않습니다(그 사람의 1주차가 뒤로 밀립니다). 이벤트가 하나도 없는 참여자는
    로그에 없으므로 아예 집계에 들어오지 않습니다 — **ITT 가 아니라 as-observed 분석**입니다.
    (3) 로그가 없는 날이 "안 썼다"인지 "기기가 꺼져 있었다"인지는 구분하지 못합니다.
    (4) `--group-col` 을 함께 쓰면 리포트 상단의 전체 준수도에는 **군 라벨이 없는 사용자도
    포함**되므로, 군별 분모의 합과 다를 수 있습니다.
...
- **개인정보(PII)**: 리포트(텍스트·JSON)에는 입력의 **사용자 ID가 그대로** 담깁니다(상위 사용자,
  users 배열, `--adherence-days` 를 주면 `adherence_users.csv`/JSON `adherence.users[]` 까지).
  **`--anonymize` 를 주면 사용자 ID를 `U001…` 가명으로 바꿔 출력**하므로 리포트를 그대로
  공유할 수 있습니다(지표 수치는 완전히 동일). 번호는 (첫 활동 시각, 원래 ID) 순이라 같은
  입력이면 항상 같은 가명이 나오고, **대응표는 어디에도 저장하지 않습니다** — 저장하는 순간
  그 파일이 다시 식별 정보가 되기 때문입니다(원본 로그에 같은 옵션을 다시 돌리면 재현됩니다).
  주의: 가명화는 **ID만** 가립니다. 이벤트 이름·군 라벨·타임스탬프는 그대로이므로 드문
  이벤트나 인원이 적은 군은 여전히 개인을 좁힐 수 있습니다(k-익명성을 보장하지 않습니다).
  `--group-col` 을 쓰면 여기에 더해 **군 라벨, 군별 인원·중앙값, KM 곡선의
  ...
  (심볼릭 링크 포함).
- 네트워크 전송·외부 호출이 전혀 없습니다(오프라인, 표준 라이브러리만). 입력 파일을 읽고
  `--out` 경로와 `--csv-dir` 폴더에만 씁니다. 표에 실리는 사용자 ID·이벤트 이름·군 라벨은
  입력에서 그대로 오므로, `=`/`+`/`@` 로 시작하는 값은 엑셀이 수식으로 실행하지 않도록
  앞에 작은따옴표를 붙여 내보냅니다(값 자체는 보존).
### 군 비교 (`--group-col`)
...
- **군 배정**: 한 사용자에게 서로 다른 라벨이 붙어 있으면 **가장 이른 이벤트의 라벨**로 확정하고
  ...
  (전체 지표에는 그대로 포함됩니다).
- **준수도 비교** (`--adherence-days` 를 함께 준 경우): 두 개의 검정이 family 에 추가됩니다 —
  **준수 참여자 비율**(비율 비교: 위험차 + Newcombe CI + Fisher)과 **사용자당 준수 주 비율(%)**
  (분포 비교: Mann-Whitney). 관찰 종료일은 전체 데이터의 마지막 날로 통일해 '완전히 관찰된 주'
  의 수가 군 간에 공정하도록 맞춥니다. 군별 준수 참여자 수는 군이 3개 이상이어도 표 아래
  한 줄로 표시됩니다.
...
- **다중비교**: 위 검정들을 **하나의 family** 로 묶어 **Holm–Bonferroni** 보정한 `p(Holm)` 을
  ...
  `--funnel`·`--adherence-days` 지정 여부에 따라 달라지므로, 같은 데이터라도 옵션을 바꾸면 `p(Holm)` 이
...
`retention`, `funnel`, `activity`, `adherence`, `groups`.

`adherence` 는 `--adherence-days` 를 준 경우에만 채워지고(아니면 `null`), 설정값
(`min_days`, `period_days`, `target`, `window_weeks`, `observed_weeks`)과 전체 요약
(`n_users`, `n_adherent_users`, `adherent_rate`, `adherent_ci`, `median_user_rate`,
`median_streak_weeks`, `n_no_full_week`, `n_incomplete`, `required_weeks`,
`eligible_weeks_range`, `notes`), 그리고 `weeks[]`(`week`, `eligible`, `adherent`, `rate`,
`ci`, `median_active_days`)와 `users[]`(`user`, `eligible_weeks`, `adherent_weeks`, `rate`,
`longest_streak_weeks`, `active_days_in_window`)를 가집니다.

`groups` 는 `--group-col` 을 준 경우에만 채워지고(아니면 `null`), 아래 키를 가집니다.
...
| `arms[]` | 군별 `n_users`·`n_events`·`n_sessions`·각종 `median_*`·`retention`·`funnel_completion`·`adherence` |
