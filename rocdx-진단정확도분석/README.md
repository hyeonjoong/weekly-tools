...
| 두 검사 비교 | **DeLong 짝지은 검정**(같은 대상) — 차이·신뢰구간·z·p (분산을 추정할 수 없으면 p를 내놓지 않고 그렇다고 말합니다) |
...
| 출력 | 텍스트 보고서 · ASCII ROC 곡선 · 마크다운 표(`--markdown`) · 절단점 전체 CSV(`--points-csv`) · 한국어/영어 논문 문장 |
...
- **경험적(비모수) ROC만** 계산합니다. 이항정규(binormal) 모형 적합, 곡선 평활화,
  **부분 AUC(pAUC)** 는 제공하지 않습니다.
- **한 행 = 서로 독립인 한 대상**을 가정합니다. 같은 환자에서 반복 측정한 자료,
  다기관 군집 자료는 신뢰구간이 실제보다 좁게 나옵니다(군집 보정 없음).
...
- p값과 신뢰구간은 **정규근사**에 기반합니다. 한 군의 사례가 10명 미만이면
  경고가 뜨고, 그때 숫자는 참고용으로만 보세요.
- **의료기기가 아니며 개별 환자의 진료 판단에 사용해서는 안 됩니다.** 연구용
  분석 도구입니다.
- 다중 검사·다중 절단점을 여러 번 비교할 때의 **다중비교 보정은 하지 않습니다.**
...
두 파일 모두 **난수로 만든 합성 데이터**이며 실제 환자 정보가 아닙니다.
...
`tests/` (187개)는 통계 핵심을 실제로 검증합니다: DeLong 분산 대 O(n²) 정의식,
