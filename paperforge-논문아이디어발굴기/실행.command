...
echo " 사용: paperforge <매니페스트.json|.csv> [--top N --out ideas.md --csv ideas.csv"
echo "        --json ideas.json --html ideas.html --dropout 0.2 --effect-scale 0.7"
echo "        --n-tests 5 --repeats 3 --icc 0.3 --one-sided --max-n 300"
echo "        --feasible-only --templates 내팩.json]"
echo " 임상 종점(반응률·생존·용량군 ANOVA·기저값 보정 ANCOVA)도 사이징:"
echo "        --templates examples/clinical_pack.json"
echo "------------------------------------------------------------------"
...
echo " 템플릿 목록: python3 -m paperforge.cli --list-templates"
echo " (설치했다면 'paperforge --list-templates' 로도 됩니다)"
echo " 자세한 안내: 사용법.md"
echo "------------------------------------------------------------------"
