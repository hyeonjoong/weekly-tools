...
- **`--interim/--spending/--timing` 있음**: `ttest2` `paired` `onesample` `prop2` `corr` `noninf` `repeated` `survival` `mcnemar` `pilot`. 없는 설계 — `anova`(전방향 F 검정) `equiv`(TOST) `prop1`(정확 이항검정) `crossover`(시기·이월효과)
...
`--spending obf`(기본, 초반을 아껴 최대 표본수가 거의 안 늘어남) · `pocock`(초반에 넉넉히 써 조기중단 확률이 높음) · `linear`. 중간분석 시점은 `--timing 0.4,0.7`처럼 정보비율로 지정합니다(기본은 균등 배치). 프로토콜 문장에도 경계값과 팽창계수가 함께 들어갑니다.
...
- 중간분석(`--interim`)의 경계·팽창계수는 **Z 통계량(정규근사)** 기반이며(gsDesign·EAST와 같은 방식), **효능 조기중단만** 반영합니다. 무익성(futility) 경계는 지원하지 않으며, 함께 쓰면 검정력이 조금 내려갑니다. `anova`·`equiv`에는 적용되지 않습니다.
...
```bash
python3 -m pytest tests/ -q     # 960개 테스트, 완전 오프라인
```
