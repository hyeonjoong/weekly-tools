...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--tau2", choices=TAU2_METHODS + tuple(m.lower() for m in TAU2_METHODS),
                   default="DL", help="변량효과 tau² 추정법 (기본: DL, 대안: PM=Paule–Mandel)")
    ...
    p.add_argument("--no-bias", action="store_true", help="Egger 출판편향 검정 생략")
    ...
    p.add_argument("--out", "-o", metavar="파일", default=None,
                   help="결과를 파일로 저장 (확장자가 .json/.md면 형식 자동 선택)")
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    if args.json and args.markdown:
        parser.error("--json 과 --md 는 함께 쓸 수 없습니다.")
    ...
    if args.out:
        ...
        if os.path.splitext(args.out)[1].lower() in (".csv", ".tsv", ".xlsx", ".xls"):
            sys.stderr.write(
                "경고: --out 확장자가 표 형식이지만 저장되는 내용은 텍스트/마크다운 리포트입니다.\n"
            )
    ...
    fmt_choice = "json" if args.json else ("md" if args.markdown else "text")
    if args.out:
