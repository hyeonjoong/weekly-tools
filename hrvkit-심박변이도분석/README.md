...
### 파일럿 → 본시험 표본수 설계 / Sample-size planning (`--power`, `--plan`)

파일럿을 돌린 다음 반드시 나오는 질문은 **"그래서 본시험은 몇 명인가"** 입니다.
`--paired`/`--groups` 에 `--power` 를 붙이면 같은 실행에서 같은 전처리로 나온
효과크기·SD 로 **다음 시험의 필요 표본수**를 함께 냅니다.

```bash
hrvkit --paired manifest.csv --power                       # 목표 검정력 80%
hrvkit --paired manifest.csv --power --target-power 0.9    # 90%
hrvkit --paired manifest.csv --power --dropout 0.15        # 탈락률 15% → 모집 인원
hrvkit --groups arms.csv     --power                       # 평행군: **군당** N
```

실제 출력 (`examples/paired/manifest.csv`, 합성 10명 · `--dropout 0.15`):

```
[C] 표본수 설계 / Sample-size planning — **다음(본)시험용**
  파일럿 n = 10, 목표 검정력 80%, α=0.05 양측, 탈락률 15%
  metric               d(관측)        N(관측)→모집   d(CI경계)        N(보수적)→모집
  ----------------------------------------------------------------------------------
  RMSSD (ms)             8.027                6‡→8       7.373                  6‡→8
  pNN50 (%)              1.295                8→10       0.642                 24→29
  LF/HF                 -1.866                 6→8      -1.212                  9→11
  SampEn                -1.275                8→10      -0.621                 25→30
  ...
```

(이 예제는 **합성 데이터라 효과크기가 비현실적으로 큽니다** — d≈8 은 실제 임상
자료에서 나오지 않습니다. 형식 시연용이며 이 표의 숫자는 참고치가 아닙니다.
실제 HRV 개입 연구의 dz 는 대개 0.3–0.9 이고, 그 범위의 필요 N(순위검정 기준)은
**dz 0.3 → 95명, 0.5 → 36명, 0.9 → 13명** 으로 7배 넘게 벌어집니다.)

#### 표의 각 열

- **d(관측)** — 파일럿에서 관측된 효과크기에 **Hedges 소표본 편의 보정** J(자유도)를
  적용한 값입니다(짝지은: dz×J(n−1), 평행군: Hedges g). 표본 효과크기는 모집단
  효과를 과대추정하므로 보정하지 않으면 N 이 **작게** 나옵니다 — 파일럿 n=5,
  dz=0.8 이면 보정 없이 15명, 보정하면 22명입니다.
- **d(CI 경계)** — 평균차 양측 CI 의 0 쪽 경계, 즉 **97.5% 단측 신뢰한계**를 SD로
  나눈 값입니다(Browne 1995 / Kieser & Wassmer 1996 의 신뢰한계 접근).
- **N** — **완료 인원**입니다. 양측 t 검정 기준 N 을 비중심 t 분포를
  **수치적분(Simpson, 오차 1e-9 이내)** 해 구한 뒤, hrvkit 이 실제로 쓰는
  Wilcoxon/Mann–Whitney 의 **정규분포 하** 점근상대효율(ARE = 3/π ≈ 0.955)을
  반영해 +4.7% 합니다. `--dropout` 을 주면 `⌈N/(1−탈락률)⌉` 로 **모집 인원**을
  함께 냅니다. 평행군은 **군당** 인원입니다(총 2N).
- **‡** — 정확검정 하한으로 올린 값. 짝지은 부호순위의 최소 양측 p 는 2^(1−n)
  이라 α=0.05 에서 **n ≤ 5 는 효과가 아무리 커도 기각이 불가능**합니다(실제
  검정력 0). Mann–Whitney 는 군당 3명 이하가 그렇습니다. t 계산이 더 작은 N 을
  내도 그 설계는 쓸 수 없으므로 하한(짝지은 6명 / 군당 4명)으로 올립니다.
- **'—'** — N 을 낼 수 없다는 표시이며 원인은 (1) CI 가 0을 포함해 효과 방향조차
  미확정, (2) 관측 효과크기가 정확히 0, (3) 효과가 너무 작아 상한 100,000명으로도
  목표에 못 미침 — 셋 중 하나입니다. 숫자를 지어내지 않습니다.

#### 두 N 은 **범위**로 읽으세요 (중요)

관측 N 은 낙관적이라 사실상 **하한**이고, CI 경계 N 은 97.5% 한계라 **상한**입니다.
Browne 1995 / Kieser & Wassmer 1996 은 97.5% 한계가 **과도하게 크게 나온다**고
보고하며 60–80% 단측 한계를 권합니다. 현실적인 파일럿에서 실제로 이렇게 벌어집니다:

| 파일럿 | dz | N(관측) | N(보수적) |
|---|---|---|---|
| n=20, Δ=4 ms, SD=8 | 0.50 | 39 | **8,722** |
| n=12, Δ=6 ms, SD=8 | 0.75 | 20 | **726** |
| n=10, Δ=7 ms, SD=9 | 0.78 | 19 | **2,528** |

보수적 N 이 이렇게 튀면 그것은 설계값이 아니라 **"이 파일럿만으로는 표본수를 정할
수 없다"** 는 신호입니다. 그 경우 선행 문헌값이나 **임상적 최소 의미차(MCID)** 를
`--plan --delta` 에 넣어 설계하세요.

#### 반드시 알아야 할 한계

- **다중비교와 α 가 어긋납니다.** `[C]` 표는 **보정 없는 α** 로 계산되는데, `[B]`
  블록은 11개 지표에 Holm/BH 를 겁니다. 보정된 기준으로 검정할 계획이라면 필요 N 이
  **약 1.7배**로 늘어납니다(dz=0.5: α=0.05 에서 34명 → α=0.05/11 에서 59명; 34명
  설계의 Holm 하 실제 검정력은 약 46%). 주 평가변수 하나를 **사전 지정**하거나
  `--alpha 0.0045`(=0.05/11)로 다시 계산하세요. `--alpha` 는 `--power`/`--plan`
  양쪽의 설계 α 로 그대로 들어갑니다.
- **파일럿의 SD 자체도 불확실합니다.** 두 N 모두 평균차의 불확실성만 반영합니다.
  n=10 이면 σ 의 95% CI ≈ [0.69ŝ, 1.83ŝ] 이고 N ∝ 1/d² ∝ ŝ² 이므로, 그것만으로
  3배 가까이 흔들립니다.
- **양측 우월성(superiority) 설계 전용입니다.** 단측·비열등성(NI)·동등성 설계에는
  여기 나온 N 을 그대로 쓰면 안 됩니다.
- **`--dropout` 은 완료자 기준 보정**입니다(탈락자가 아무 정보도 주지 않는다는 가정).
  ITT 분석에는 그대로 쓸 수 없습니다.
- **HF power 처럼 오른쪽으로 크게 치우친 지표**는 원 척도의 평균/SD 로 낸 N 이 잘
  맞지 않습니다 — `ln_hf` 로 설계하는 편이 안전합니다.
- **ARE 는 점근값이고 정규분포 기준**입니다. 최악의 연속분포에서는 ARE 가 0.864 까지
  내려가 +15.7% 가 필요할 수 있습니다. 또 순위검정의 귀무가설은 평균차가 아니라
  확률적 순서(HL 이동량)이므로, 평균/SD 로 만든 d 에 ARE 를 곱하는 것은 근사적인
  다리입니다.
- **`[C]` 의 CI 는 모수적 t 신뢰구간**입니다 — `[B]` 표의 CI 는 분포무관
  Hodges–Lehmann 구간이라 같은 자료에서도 값이 다릅니다.
- **사후 검정력(observed/post-hoc power)은 계산하지도 보고하지도 않습니다.** 관측된
  효과를 그대로 넣은 검정력은 p값의 단조함수라 새 정보가 없고("p가 작으면 검정력이
  높다"는 동어반복), 유의하지 않은 결과의 사후 변명으로 오용됩니다. 다만 N(관측)도
  관측 효과크기의 단조함수이므로 같은 한계를 공유한다는 점은 밝혀 둡니다.

#### 파일럿 없이 설계 (`--plan`)

가정값만으로 설계표를 만듭니다. 파일 인자를 받지 않습니다.

```bash
# RMSSD 8 ms 차이를, 개인 내 차이의 SD 15 ms 가정으로 탐지하려면?
hrvkit --plan --delta 8 --sd 15
# 확보 가능한 인원이 군당 24명일 때 무엇을 탐지할 수 있나? (MDD)
hrvkit --plan --plan-n 24 --sd 15 --design parallel
# 둘 다 주면 그 조합의 검정력까지 (text/JSON/CSV 모두 power_at_n 포함)
hrvkit --plan --delta 8 --sd 15 --plan-n 30 --dropout 0.1 --format csv
# 표준 격자(80/85/90/95%)에 없는 목표도 --target-power 로 추가됩니다
hrvkit --plan --delta 8 --sd 20 --target-power 0.99
```

```
  [필요 표본수]
    목표 검정력        N(t 검정)     N(순위검정)
    --------------------------------------------
    80%                       30              32  ◀ --target-power
    85%                       34              36
    90%                       39              41
    95%                       48              51
```

`--sd` 의 의미는 설계에 따라 다릅니다 — **paired 면 개인 내 차이(post−pre)의 SD**,
**parallel 이면 군 내 합동 SD** 입니다. 짝지은 설계에 집단 SD 를 넣으면 필요
표본수가 크게 과대추정되므로 리포트가 이 점을 매번 명시합니다. `--plan-n` 을 주면
그 인원에서 탐지 가능한 최소 차이(MDD)를 t 기준과 순위검정 기준(×1/√0.955 ≈ +2.3%)
두 열로 냅니다.

#### CSV 출력 형태 (`--power --format csv`)

표본수 계획표는 통계표와 열 구성이 달라 **하나의 CSV 로 합칠 수 없습니다.** 주
통계표를 먼저 내보내고(그래서 `head -1` 이 주 헤더), 아래 구분선 뒤에 계획표를
붙입니다:

```
metric,psd_method,n,mean_base,...        ← 주 통계표 (11행)

# ---- sample-size planning (표본수 설계) ----
metric,design,n_pilot,mean_diff,...      ← 표본수 계획표 (11행)
```

`awk '/^# ----/{f=1;next} !f' out.csv` 로 주 표만, `awk '/^# ----/{f=1;next} f'`
로 계획표만 뽑을 수 있습니다. `--plan --format csv` 는 표가 하나뿐이라 그대로
파싱됩니다.

#### 정확성 검증

비중심 t CDF 는 ncp=0 에서 기존 `student_t_cdf` 와 **|t| ≤ 1e5 전 범위에서 5e-11**
이내로 일치하고, `t=0` 에서 Φ(−δ), 반사 항등식 `F(t;δ)=1−F(−t;−δ)` 를 만족합니다
(Φ 의 전이 구간을 경계로 적분 구간을 쪼개 두 스케일이 아무리 달라도 계단을
표본합니다). 필요 표본수는 R `stats::power.t.test` 와 6개 조건에서 일치합니다
(paired d=0.5→34, d=0.2→199, d=1.0@90%→13; parallel d=0.5→64, d=0.8→26,
d=0.3→176). 검정력 자체는 시뮬레이션 20,000회 t 검정으로도 재현합니다.
...
| `--alpha 0.05` | 유의수준 — HL 신뢰구간 수준(1-α), 유의 판정 기준, **그리고 `--power`/`--plan` 의 설계 α**. 다중비교 보정 기준으로 설계하려면 여기를 낮추세요(예: `--alpha 0.0045` = 0.05/11) |
| `--power` | `--paired`/`--groups` 결과에 **다음 시험 표본수 설계** 블록 추가 (관측 효과 + 97.5% 단측 신뢰한계 기준, 두 N 을 **범위**로 읽음) |
| `--target-power 0.80` | 표본수 설계의 목표 검정력. `--plan` 에서는 표준 격자(80/85/90/95%)에 이 값이 추가됩니다 |
| `--dropout 0.0` | 예상 탈락률 → 완료 인원 N 에 더해 **모집 인원** `⌈N/(1−FRAC)⌉` 보고 (완료자 기준 보정 — ITT 아님) |
| `--plan` | 파일럿 없이 가정값만으로 설계 (파일 인자를 받지 않음) |
| `--delta D` | `--plan`: 탐지하려는 차이(원 단위) |
| `--sd S` | `--plan`: 가정 SD — paired면 **차이의 SD**, parallel이면 군 내 합동 SD |
| `--plan-n N` | `--plan`: 확보 가능한 표본수(parallel이면 군당, 상한 10,000,000) → 탐지 가능한 최소 차이(MDD) |
| `--design paired\|parallel` | `--plan` 의 설계 (기본 paired) |
