...
def gammainc_upper(a: float, x: float) -> float:
    ...
    Computed directly (series for x < a+1, continued fraction otherwise) rather
    than as ``1 - gammainc_lower``. The subtraction form loses all significance
    once P(a, x) rounds to 1.0, which for the deep right tail (large chi-square /
    tiny p-value) collapsed the result to exactly 0.0; the continued fraction
    returns the true tiny probability instead (e.g. chi2_sf(200, 10) ->
    1.6e-37, not 0.0).
    """
    if x < 0.0 or a <= 0.0:
