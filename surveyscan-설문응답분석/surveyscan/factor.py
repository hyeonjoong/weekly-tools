...
# PAF 반복 상한/수렴 기준.
# PAF는 문항 수가 적을 때(특히 k=3) 선형적으로 아주 느리게 수렴한다. 상한이 300이면
# 실제 리커트 자료에서 k=3 하위척도의 약 27%가 상한에 걸려 ω가 통째로 '산출불가'로
# 떨어졌다(수렴 자체는 정상, 반복이 모자랐을 뿐 — 500~2500회면 도달). 상한을 넉넉히
# 잡아도 k가 크면 수십 회에 끝나므로 실사용 비용은 거의 없다.
_PAF_MAXIT = 5000
_PAF_TOL = 1e-9
# 멱승법 반복 상한/수렴 기준.
_POW_MAXIT = 20000
