...
echo "  · 임상 심각도 구간 · 집단 비교 · 사전-사후 변화량"
echo "============================================================"
echo
...
echo
echo "------------------------------------------------------------"
echo "▶ 사전-사후(기저 vs 12주) 예시도 이어서 실행합니다:"
echo "  python3 -m surveyscan.cli examples/sleep_prepost.csv \\"
echo "      --config examples/sleep_prepost_config.json \\"
echo "      --id-col ID --time-col 시점 --group-col 군 --nonparam"
echo
python3 -m surveyscan.cli examples/sleep_prepost.csv \
    --config examples/sleep_prepost_config.json \
...
echo "사전-사후 비교:               --time-col 시점  (--id-col 필수)"
